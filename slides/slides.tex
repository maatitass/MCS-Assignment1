...
\begin{frame}{Criterio di arresto}
    \begin{itemize}
        \item Tutti i metodi partono dal vettore iniziale nullo $\mathbf{x}^{(0)}=\mathbf{0}$.
        \item L'iterazione si ferma quando il residuo scalato scende sotto la tolleranza, oppure superato il limite di passi.
        \[ \frac{\|\mathbf{b} - A\mathbf{x}^{(k)}\|}{\|\mathbf{b}\|} < \texttt{tol} \]
    \end{itemize}
\end{frame}

\begin{frame}{Metodi Stazionari e Splitting}
    \begin{itemize}
        \item Si basano su uno \emph{splitting} della matrice $A = P - N$.
        \item Si definiscono \emph{stazionari} perché le matrici $P$ e $N$ rimangono fisse da un'iterazione all'altra.
        \item Scegliendo $P$ facile da risolvere, la formula si manipola nella forma usata nel codice:
        \[ \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} + P^{-1}\mathbf{r}^{(k)} \]
        \item La convergenza è garantita quando il raggio spettrale della matrice di iterazione $P^{-1}N$ è minore di 1.
    \end{itemize}
\end{frame}
...
\begin{frame}{Metodo di Jacobi}
    \begin{itemize}
        ...
        \item Essendo $D$ diagonale, l'inversa equivale a dividere ogni componente del residuo per l'elemento sulla diagonale.
        \item Calcolando i reciproci all'inizio, l'aggiornamento si esegue con un'unica operazione vettoriale veloce.
...
\begin{frame}{Metodo di Gauss-Seidel}
    \begin{itemize}
        \item Si prende come $P=D+L$ la parte triangolare inferiore di $A$, diagonale compresa.
        \[ \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} + (D+L)^{-1}\mathbf{r}^{(k)} \]
        \item Non si calcola mai esplicitamente l'inversa: si procede risolvendo per sostituzione in avanti (dall'alto in basso).
        \item La differenza rispetto a Jacobi è l'uso immediato dei valori appena calcolati per le righe successive.
        \item L'uso di informazioni aggiornate garantisce generalmente una convergenza più rapida rispetto a Jacobi.
    \end{itemize}
\end{frame}

\begin{frame}{Metodi del gradiente: discesa nel paraboloide}
    \begin{itemize}
        \item Per matrici SPD risolvere il sistema equivale a trovare il punto di minimo di un paraboloide convesso:
        \[ \phi(\mathbf{y})=\tfrac12\,\mathbf{y}^{t}A\mathbf{y}-\mathbf{b}^{t}\mathbf{y} \]
        \item La geometria di questo paraboloide è dettata dagli autovalori della matrice.
        \item Il numero di condizionamento $\kappa(A) = \lambda_{\max}/\lambda_{\min}$ indica quanto il paraboloide sia stretto e allungato.
    \end{itemize}
\end{frame}

\begin{frame}{Metodo del Gradiente}
    \begin{itemize}
        \item Adotta la strategia più intuitiva: a ogni passo avanza lungo la direzione più ripida in discesa, ovvero il residuo $\mathbf{r}^{(k)}$.
        \[ \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} + \alpha_k \mathbf{r}^{(k)} \]
        \item Il difetto di questa strategia emerge quando la matrice è mal condizionata (valle lunga e stretta).
        \item Il percorso compie continui rimbalzi da una parete all'altra con un andamento a \emph{zig-zag}.
        \item Il metodo avanza pochissimo verso il minimo globale, impiegando migliaia di iterazioni per smorzare l'errore.
    \end{itemize}
\end{frame}

\begin{frame}{Metodo del Gradiente Coniugato}
    \begin{itemize}
        \item Elimina il problema dello zig-zag scegliendo direzioni di ricerca $\mathbf{d}^{(k)}$ che non interferiscono tra loro.
        \item Tali direzioni sono \emph{$A$-coniugate}: una volta raggiunto il minimo lungo una direzione, nessun passo futuro lo disferà.
        \[ \mathbf{x}^{(k+1)} = \mathbf{x}^{(k)} + \alpha_k \mathbf{d}^{(k)} \]
        \item Ogni iterazione sistema definitivamente una dimensione: in aritmetica esatta convergerebbe in al più $n$ iterazioni.
        \item In pratica il numero di iterazioni cresce solo proporzionalmente a $\sqrt{\kappa(A)}$.
    \end{itemize}
\end{frame}

\begin{frame}{Architettura della libreria}
    \begin{itemize}
        \item I metodi condividono uno scheletro comune tramite la classe base astratta \texttt{IterativeSolver}.
        \item La classe base gestisce il vettore iniziale, il ciclo di arresto e il timer.
        \item La logica matematica del singolo passo è delegata a oggetti \texttt{\_Iteration} specifici per ogni sottoclasse.
        \item Il risultato è un oggetto \texttt{SolverResult} (iterazioni, tempo, residuo, convergenza).
        \item Chiamiamo \texttt{warmup()} per pagare una volta sola la compilazione JIT di \texttt{numba} e non falsare i tempi.
    \end{itemize}
\end{frame}
...
\begin{frame}{Analisi dei risultati per matrice}
    \begin{itemize}
        \item Sulla matrice \texttt{spa1} il condizionamento altissimo ($\kappa\approx2.0\cdot10^3$) penalizza fortemente il gradiente semplice, mentre Gauss-Seidel converge in pochi passi.
        \item Sulla matrice \texttt{spa2} ($n=3000$) Gauss-Seidel risulta in assoluto il più veloce, dato che il basso numero di iterazioni compensa il calcolo lento.
        \item Sulla matrice \texttt{vem1} il gradiente coniugato è la scelta più efficiente: annulla la discesa a zig-zag e domina con tempi prossimi allo zero.
        \item Sulla matrice \texttt{vem2} emerge di nuovo la robustezza del gradiente coniugato: le iterazioni sono bassissime e i tempi sono frazioni di millisecondo.
    \end{itemize}
\end{frame}
...
\begin{frame}{Analisi delle iterazioni e dell'errore}
    \begin{itemize}
        \item Il gradiente coniugato richiede pochissimi passi, mentre il gradiente semplice ne accumula migliaia sulle matrici \texttt{spa} a causa del percorso a zig-zag.
        \item Gauss-Seidel converge sempre in meno iterazioni di Jacobi perché riusa subito i valori appena aggiornati (fino a un decimo sulle \texttt{spa}).
        \item Le rette dell'errore relativo scendono in modo regolare: l'errore resta sempre proporzionale alla tolleranza con pendenza quasi uguale per tutti.
    \end{itemize}
\end{frame}

\newsavebox{\iterbox}
\sbox{\iterbox}{\includegraphics{../results/plots/iterations.png}}
...
\newsavebox{\errbox}
\sbox{\errbox}{\includegraphics{../results/plots/relative_error.png}}
...
\begin{frame}{Tempi totali di calcolo}
    \begin{itemize}
        \item Sulle matrici \texttt{spa} il più veloce è Gauss-Seidel, perché converge in pochissime iterazioni e questo compensa il costo del passo sequenziale.
        \item Sulle matrici \texttt{vem} la situazione si ribalta: il gradiente coniugato è di gran lunga il più veloce (frazioni di millisecondo), staccando nettamente gli altri tre.
        \item Il gradiente semplice si conferma di gran lunga il più lento, in particolare sulle \texttt{spa}, arrivando quasi a 10 secondi su \texttt{spa2}.
    \end{itemize}
\end{frame}

\newsavebox{\timebox}
\sbox{\timebox}{\includegraphics{../results/plots/time.png}}
...
\begin{frame}{Storia di convergenza}
    \begin{itemize}
        \item Sulle matrici \texttt{spa} Gauss-Seidel scende per primo e in modo ripido, toccando subito la tolleranza richiesta.
        \item Il gradiente semplice continua a scendere molto lentamente per migliaia di iterazioni, segno inequivocabile della convergenza lenta a zig-zag.
        \item Il gradiente coniugato ha un andamento più irregolare ma scende molto più in fretta, e nelle iterazioni finali la discesa subisce una chiara accelerata.
    \end{itemize}
\end{frame}

\newsavebox{\convbox}
\sbox{\convbox}{\includegraphics{../results/plots/convergence.png}}
...
\begin{frame}{Costo del singolo passaggio}
    \begin{itemize}
        \item Il costo di una singola iterazione cresce all'aumentare delle entrate non nulle (densità della matrice).
        \item Jacobi, Gradiente e CG hanno un costo per iterazione simile, dominato dal prodotto matrice-vettore vettorializzato.
        \item Il passo di Gauss-Seidel è più costoso a causa della sostituzione in avanti puramente sequenziale.
        \item Gauss-Seidel conviene solo quando il grande risparmio di iterazioni ripaga la lentezza del singolo passo (es. matrici \texttt{spa}).
    \end{itemize}
\end{frame}
