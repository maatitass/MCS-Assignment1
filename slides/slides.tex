...
\begin{frame}{Introduzione}
    \begin{itemize}
        ...
        \item Il lavoro è validato misurando l'errore relativo rispetto alla soluzione esatta fissa.
    \end{itemize}
\end{frame}

\section{I metodi e la libreria}

\begin{frame}{Criterio di arresto e teoria dello Splitting}
    \begin{itemize}
...
\begin{frame}{Metodo del Gradiente}
    \begin{itemize}
        ...
        \item Quando gli autovalori sono molto diversi la discesa procede a zig-zag e servono molti passi.
    \end{itemize}
\end{frame}
...
\begin{frame}{Metodo del Gradiente Coniugato}
    \begin{itemize}
        ...
        \item Come per il gradiente, si calcola un solo prodotto matrice--vettore per iterazione, riusato anche per il residuo.
    \end{itemize}
\end{frame}
...
\begin{frame}{Matrici di test}
    ...
    \end{table}
    \begin{itemize}
        \item Le matrici della famiglia \texttt{spa} sono di media densità (circa il 18\%), con un condizionamento elevato (migliaia).
        \item Le matrici \texttt{vem} sono molto sparse (sotto lo 0.5\%); hanno un autovalore minimo vicino a zero, ma il condizionamento resta nell'ordine delle centinaia.
    \end{itemize}
\end{frame}
...
\begin{frame}{Numero di iterazioni per tolleranza (Parte Superiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Numero di iterazioni per tolleranza (Parte Inferiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Errore relativo misurato (Parte Superiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Errore relativo misurato (Parte Inferiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Tempi totali di calcolo (Parte Superiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Tempi totali di calcolo (Parte Inferiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Storia di convergenza (Parte Superiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Storia di convergenza (Parte Inferiore)}
    \begin{figure}
        \centering
...
\begin{frame}{Condizionamento e Costo del singolo passaggio}
    \begin{itemize}
        ...
        \item Il rapporto calcolato tra errore relativo e residuo relativo non supera mai il confine segnato dal condizionamento.
    \end{itemize}
\end{frame}

\begin{frame}{Iterazioni e numero di condizionamento}
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{../results/plots/iter_vs_cond.png}
    \end{figure}
\end{frame}
...
\begin{frame}{Amplificazione del residuo}
    \begin{figure}
        \centering
        \includegraphics[width=\textwidth,height=0.85\textheight,keepaspectratio]{../results/plots/error_vs_residual.png}
    \end{figure}
\end{frame}

\begin{frame}{Confronto conclusivo delle performance}
    \begin{table}
        \centering
        \small
        \begin{tabular}{lcccc}
        \toprule
        Esito Finale & Jacobi & Gauss-Seidel & Gradiente & Gradiente Coniugato \\
        \midrule
        Iterazioni Totali & molte & medie & moltissime & pochissime \\
        Dipendenza Tolleranza & lineare & lineare & lineare & lineare \\
        Dipendenza $\kappa(A)$ & varia & varia & $\approx\kappa$ & $\approx\sqrt{\kappa}$ \\
        Costo per Passo & basso & alto & basso & basso \\
        Miglior velocità & Nessuno & matrici dense & Nessuno & matrici vuote \\
        Robustezza & media & media & bassa & altissima \\
        \bottomrule
        \end{tabular}
    \end{table}
\end{frame}
...
\begin{frame}{Conclusioni}
    \begin{itemize}
        \item Lo scheletro base dell'architettura fornisce un tracciamento sicuro per la validazione iterativa totale.
        \item Il vincolo di blocco sul residuo garantisce la correttezza della stima prevenendo calcoli inutili.
        \item I risultati inquadrano il gradiente coniugato come la selezione operativa più pratica per efficacia.
        \item Gauss-Seidel mantiene il proprio profilo prestazionale elevato nei casi specifici di reticoli mediamente densi.
    \end{itemize}
\end{frame}
